#! /usr/bin/env python3
"""
inspired by s2a_fm

test

"""

from http.server import BaseHTTPRequestHandler
from http.server import HTTPServer
from urllib.parse import quote, unquote

import http.client
import json
import time

import os, sys

import urllib


class WeatherQuery():

    #weather result
    city = ""
    weather = ""
    temp = None

    #city name send by scratch
    scratch_city_name = ""

    #weather http connection object
    conn = None 

    weather_site = "api.openweathermap.org"
    prefix_path = "/data/2.5/weather?q="
    postfix_path = "&units=metric&lang=zh_tw&mode=json"

    has_data = False
    
    def __init__(self):
        self.conn = http.client.HTTPConnection(self.weather_site)

    def getWeatherInfo(self, city_str):
        query_path = self.prefix_path + city_str + self.postfix_path

        self.conn.connect()
        self.conn.request("GET",query_path)

        r = self.conn.getresponse()

        content = r.read()
        data = json.loads(content.decode('utf-8'))

        self.city = data['name']
        self.weather = data['weather'][0]['description']
        self.temp = data['main']['temp']
        #print(query_path)
        #print(r.status)

        self.conn.close()
        
        if not self.has_data :self.has_data = True 

    def resultString(self):
        s = ""
        if self.has_data:
            s = "reportData/{} {}".format(quote("城市"),self.city) + "\r\n"
            s = s + "reportData/{} {}".format(quote("天氣"),quote(self.weather)) + "\r\n"
            s = s + "reportData/{} {}".format(quote("目前溫度"),round(float(self.temp),1)) + "\r\n"
            
            
        return s


class GetHandler(BaseHTTPRequestHandler):
    """
    This class contains the HTTP server that Scratch2 communicates with
    Scratch sends HTTP GET requests and this class processes the requests.

    HTTP GET requests are accepted and the appropriate command handler is
    called to process the command.
    """
    #class variable for consistent use    
    
    weather_query = None

    @classmethod
    def set_items(cls, weather):
        cls.weather_query = weather
    

  
    def do_GET(self):
        """
        Scratch2 only sends HTTP GET commands. This method processes them.
        It differentiates between a "normal" command request and a request
        to send policy information to keep Flash happy on Scratch.
        (This may change when Scratch is converted to HTML 5
        """

        # skip over the / in the command
        cmd = self.path[1:]

        # create a list containing the command and all of its parameters
        cmd_list = cmd.split('/')
        
        s = "okay"

        #handle string s , and cal send_resp
        
        

        
        #print command text
        if not cmd.startswith("poll"): print ("[query]: " + self.path)

        if cmd.startswith("weatherQuery") :
            GetHandler.weather_query.getWeatherInfo(cmd_list[2])

            
        if  cmd.startswith("poll"):
            s = GetHandler.weather_query.resultString()

        self.send_resp(s)


    def send_resp(self, response):
        """
        This method sends Scratch an HTTP response to an HTTP GET command.
        """

        crlf = "\r\n"
        http_response = "HTTP/1.1 200 OK" + crlf
        http_response += "Content-Type: text/html; charset=ISO-8859-1" + crlf
        http_response += "Content-Length" + str(len(response)) + crlf
        http_response += "Access-Control-Allow-Origin: *" + crlf
        http_response += crlf
        #add the response to the nonsense above
        if response != 'okay':
            http_response += str(response + crlf)
        # send it out the door to Scratch
        self.wfile.write(http_response.encode('utf-8'))
        

def start_server():
    """
       This function populates class variables with essential data and
       instantiates the HTTP Server
    """
    port = 50444

    GetHandler.set_items(WeatherQuery())
    
    try:
        server = HTTPServer(('localhost', port), GetHandler)
        print ('Starting Scratch HTTP Server!(port ' + str(port) + ')')
        print ('Use <Ctrl-C> to exit the extension\n')
        print ('Please start Scratch !')
    except Exception:
        print ('HTTP Socket may already be in use - restart Scratch')
        raise

    try:
        #start the server
        server.serve_forever()
    except Exception:
        raise
        
if __name__ == "__main__":

        start_server()
